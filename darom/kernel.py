...
class Kernel:
    ...
    def _print_process_transition(self, old_proc):
        print("{:<15} -> {:>15}".format(old_proc.name if old_proc else "",
                                        self._run_proc.name))
    ...
    def _select_process(self):
        ...
        candidates = [
            proc for proc in self._ready_procs if proc.priority == max_p]
    ...
    def planner(self, window=None, init=None):
        while True:
            ...
            if self._ready_procs:
                ...
                self._run_proc = p
                self._print_process_transition(
                    old_proc) if self._verbose else None
                p.run()
            else:
                self._run_proc = process.Process(name='IDLE', priority=10)
            if window:
        ...
        for res in self._resources:
            for req in res._wait_list:
                if res.elements_available(req.amount, req.cond):
                    req.proc.owned_res.extend(
                        res.get_elements(req.amount, req.cond))
                    req.proc.unblock()

    def create_process(self, process):
        print(
            "{:<15} : create_process({})".format(
                self._run_proc.name,
                process.name)) if self._verbose else None
    ...
    def request_res(self, res_name, amount, cond=None):
        print(
            "{:<15} : request_res({})".format(
                self._run_proc.name,
                res_name)) if self._verbose else None
    ...
    def release_res(self, res_name, elem_data):
        '''
        elem_data - list that contains data of all resource elements
        '''
        print(
            "{:<15} : release_res({}, {})".format(
                self._run_proc.name,
                res_name,
                elem_data)) if self._verbose else None
